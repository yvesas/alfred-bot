# Roadmap — bot-telegram joaquim252
...
- ✅ CI **bot** (lint + type-check + testes com cobertura, Codecov) e **web** (lint + type-check + testes + build), path-filtered, `--frozen-lockfile`
- ✅ **ESLint no web** (flat config + react-hooks + prettier) — antes o web não tinha lint
- ✅ Husky cobre **os dois projetos**: pre-commit roda lint-staged + type-check por projeto (conforme arquivos staged); pre-push roda os testes de bot **e** web
- ✅ CI do bot **cacheia o binário do MongoDB** (`mongodb-memory-server`) — evita o download a cada run
