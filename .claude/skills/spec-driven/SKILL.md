...
metadata:
  ...
  version: 2.0.0
  adapted-by: YAS Labs — pasta `specs/` (visível) no lugar de `.specs/`; ver "YAS Labs" abaixo
---
...
## YAS Labs — como esta skill se encaixa no workspace
...
- **`specs/` é o plano; `docs/` é o produto acabado.** Requisito, decisão em aberto,
  ...
  runbook, API e onboarding vivem em `docs/`. Ver `.claude/rules/docs-e-specs.md`.
...
- **O gate de cada task** (`Done when` / `Gate`) usa os comandos do `CLAUDE.md`
  do projeto. Se o projeto ainda não tem `specs/codebase/TESTING.md`, pergunte os
  comandos de teste antes de criar tasks — não invente.
